## 目录

[TOC]

# 1 前言

## 1.1 实习背景和目的

&emsp;&emsp;随着信息技术的飞速发展，软件产业在我国经济中的地位日益凸显。软件工程专业作为培养高素质软件人才的重要途径，肩负着为我国软件产业输送新鲜血液的重任。在大学期间，理论知识的学习为我们打下了扎实的专业基础，然而，实践能力的培养同样至关重要。为了更好地将所学知识应用于实际工作中，提高自己的专业技能和实际操作能力，根据学校教学计划，我们需完成为期两周的校内实习任务。

&emsp;&emsp;实习是一种实践。是理论联系实际，应用和巩固所学专业知识的一项重要环节，是培养学生能力和技能的一个重要手段。实习能够将所学的软件工程理论知识与实际项目相结合，提高自己的项目分析和设计能力，锻炼编程和测试技能；学会在实际项目中运用软件工程方法，如需求分析、系统设计、编码实现、测试与维护等，提高项目开发效率和质量；增强沟通与协作能力，培养自己在团队中的责任感和使命感，为今后的职业生涯积累宝贵经验；通过实习，发现自己在专业知识和技能方面的不足，以便在今后的学习和工作中加以改进和提高。

## 1.2 实习环境

&emsp;&emsp;实习时间：8月27日-9月7日

​		实习地点：西北工业大学

# 2 实习内容与过程

## 2.1 项目概述

### 2.1.1 需求说明

#### 2.1.1.1 项目介绍

&emsp;&emsp;共享云盘是互联网存储工具，适用于PC电脑、安卓手机，是互联网云技术的产物，该系统提供了注册、登录、文件上传、文件列表、文件下载、文件分类管理、文件复制与移动、文件夹管理、文件查询功能、文件外链分析、在线预览文件、个人分享列表、回收站等基础功能，另外扩展了用户组和介质管理功能。允许用户创建用户组并邀请其他用户加入，拥有共同权限查看、修改用户组中所加载介质中的文件。介质管理功能中用户可以添加基于S3协议的对象存储服务、基于WebDAV、FTP、SFTP协议的文件传输服务等介质，进行存储文件，并且介质的所属权可以归属用户也可以属于用户组。本项目使用软件开发生产线（`CodeArts`）平台来管理项目的规划、进度跟踪、代码托管、代码检查、编译构建、流水线部署、测试与发布等软件开发全过程，解决需求变动频繁、开发测试环境复杂、多版本分支维护困难、无法有效监控进度和质量等研发痛点。

#### 2.1.1.2 软件项目概述

项目名称：共享云盘

项目开发环境：
- Windows 10/11
- IDE: IntelliJ IDEA 2023.3.4专业版
- Maven: 3.64
- Springboot: 2.6.13
- JDK: 1.8

项目运行环境：
- Ubuntu 22.04
- JRE: 1.8
- 缓存: Redis
- 持久化数据库: MySQL
- 文件系统: MinIO

项目功能：用户注册登录功能、文件上传下载功能、文件/文件夹分类添加移动删除功能、文件查询功能、文件外链分享功能、介质管理功能、回收站功能

用途：共享云盘将传统的本地磁盘存储转移到网上存储，其特点如下：

安全保密：密码和手机绑定、空间访问信息随时告知；

超大存储空间：不限单个文件大小，最多支持无限独享存储空间（视服务器而定）；

好友共享：通过链接提取码轻松分享；

在线预览：无需下载至本地。

介质访问：用户可以将自己的磁盘空间等介质上传到客户端，供其他用户访问。

#### 2.1.1.3 软件功能概述

| 功能 | 优先级 | 负责人 | 描述0 |
|--|--|--|--|
|用户注册|高|杨宏斌（后端）、唐新（前端）|用户通过注册页面能够注册账号|
|用户登陆|高|杨宏斌（后端）、唐新（前端）|用户根据已有账号进行登录|
|用户组创建|高|杨宏斌（后端）、唐新（前端）|每一个用户可以创建多个用户组，用户组拥有一个根目录，可以给用户组内的用户分配存储份额，并且每个用户可以访问用户组目录中存储文件|
|用户加入用户组|高|杨宏斌（后端）、唐新（前端）|用户可以被另一个用户邀请加入用户组|
|文件上传|高|杨宏斌（后端）、唐新（前端）|用户可以将文件上传至当前目录下，支持离线下载功能|
|文件下载|高|杨宏斌（后端）、唐新（前端）|用户可以将当前目录下的文件下载到个人磁盘空间中|
|新建文件夹|高|杨宏斌（后端）、唐新（前端）|用户在当前目录下新建文件夹|
|文件/文件夹重命名|高|杨宏斌（后端）、唐新（前端）|用户可以给文件/文件夹重命名|
|删除文件/文件夹|高|杨宏斌（后端）、唐新（前端）|用户可以删除当前目录下指定文件/文件夹|
|修改文件/文件夹权限|高|夏天（后端）、唐新（前端）|用户能够修改某个用户组文件/文件夹权限，让其他用户|
|外链分享|高|杨宏斌（后端）、唐新（前端）|用户可以分享链接供他人预览、下载文件/文件夹|
|文件查询|中|杨宏斌（后端）、唐新（前端）|用户可以按文件/文件夹名称查找对应文件|
|回收站|中|夏天（后端）、唐新（前端）|用户可以将文件/文件夹放入回收站，保存期限为15天，可以还原/删除回收站中某个文件/文件夹，也可以清空回收站|
|添加介质|高|夏天（后端）、唐新（前端）|用户可以添加WebDAV、SFTP、阿里云OSS等介质放入用户组|
|删除介质|高|夏天（后端）、唐新（前端）|用户可以删除某一介质|
|在线预览图片/txt功能|中|夏天（后端）、唐新（前端）|用户点击预览按钮进行预览图片、文本文件功能|
|在线播放音视频功能|中|夏天（后端）、唐新（前端）|用户点击预览能直接播放音视频|
|在线预览office文档|中|夏天（后端）、唐新（前端）|用户点击预览能直接预览office文档|
|文件分类|低|夏天（后端）、唐新（前端）|用户能够按文件类别名称分类，点击视频、音频、文本文件等按钮可以显示分类后的文件|

#### 2.1.1.4 软件需求分析模型

- 用例图

![](用例图.png)

- ER 图

```mermaid
erDiagram
    Account }|--o{ Account : "user participate in group"
    Account ||--o{ Storage : "Has"
    Account ||--|| Directory : "Has as Root"
    Directory |o--o{ Directory : "Contains"
    Storage |o--o{ Directory : "Mount on"
    Directory ||--o{ SymbolicLink : "Contains"
    SymbolicLink }o--|| Path : "Link to"
    Account ||--o{ SharedPath : "Share"
    Storage ||--o{ RemovedPath : "Move to Trash"
```

#### 2.1.1.5 性能要求

&emsp;&emsp;对于非文件传输类请求，系统应在1秒内给出相应，对于文件传输类请求，在系统到客户端、系统到用户存储介质之间的网络通畅可靠的情况下，应保证至少200KB每秒的传输速率。

#### 2.1.1.6 设计约束

1. 开发工具约束：
   - 开发语言：Java（后端）、HTML、CSS、Javascript（前端）
   - 开发软件：IntelliJ IDEA 2023.3.4专业版、vscode 
   - 后端框架：Spring Boot + Spring MVC + MyBatis + MinIO
   - 前端框架：Vue3
2. 运行环境约束：
   - 服务器操作系统：Ubuntu 22.04
   - 客户端：Web网页端 、Electron桌面端、Android移动端
   - 操作系统：Windows 10/11
3. 安全性要求：
   - 用户认证：需要实现用户登录功能，可能涉及权限控制。
   - 安全保密：密码和手机绑定、空间访问信息随时告知；
   - 安全合规性：符合相关法律法规对数据保护的要求。
4. 可靠性要求：
   - 系统稳定性：能够持续稳定运行，具备故障容忍性和备份恢复能力。
   - 超大存储空间：不限单个文件大小，最多支持无限独享存储空间（视服务器而定

#### 2.1.1.7 界面要求

- 在设计上，用户界面应简洁直观，符合用户习惯，易于操作和理解，减少用户学习成本从而提升用户体验。
- 在功能上，各功能模块应明确分组并提供清晰的导航菜单以方便用户快速找到所需要的功能以及切换不同功能页面。
- 在美观上，界面应尽可能使用图片、按钮、选择器等组件并设置合适的样式以实现功能需求和美观需求。

#### 2.1.1.8 交付要求

1. 软件产品交付
   内容：完整的应用软件。
2. 文档交付 
   内容：需求分析文档、系统设计文档、项目思维导图、项目甘特图
3. 源代码交付：
   内容：系统的源代码，包括前端、后端以及数据库代码。
   
### 2.1.2 概要设计

#### 2.1.2.1 体系结构设计

![](体系结构设计.png)

#### 2.1.2.2 功能结构设计

![](功能结构图.png)

#### 2.1.2.3 用户界面设计

![](我的文件-目录内.png)

![](我的文件-搜索.png)

![](我的文件-用户组选择.png)

### 2.1.3 数据库设计说明

#### 2.1.3.1 联表查询逻辑设计

```mermaid
graph LR
    user((user))---user_user_info["从user表中剔除用户组"]---user_info((user_info))
    user---user_participation["查询用户组中的成员"]---participation((participation))
    user---participation_user["查询用户加入的用户组"]---participation
    user---user_storage["查询用户（组）注册的介质"]---storage((storage))
    user---user_filesystem["查询用户（组）的根目录"]---filesystem((filesystem))
    filesystem---fs_fs["查询文件夹的父文件夹"]---filesystem
    filesystem---fs_mount["查询文件夹上挂载的介质"]---mount((mount))
    storage---storage_mount["查询介质的全部挂载点"]---mount
    symbolic_link((symbolic_link))---link_fs["查询符号链接所在的文件夹"]---filesystem
    symbolic_link---link_user["查询符号链接的目标用户"]---user
    storage---storage_recycle["查询介质的回收站内容"]---recycle((recycle))
    sharing((sharing))---sharing_user["查询链接分享的发起者"]---user
    invitation((invitation))---invitation_user["查询用户组的全部邀请链接"]---user
    admin((admin))
```

#### 2.1.3.2 表汇总

|表名|中文名称|描述|
|---|---|---|
| user | 用户（组）表 | 记录用户和用户组名称的表，合并用户和用户组两个实体是为了方便文件和介质根据所有者联表查询 |
|user_info|用户信息表|记录用户信息，与user联表查询后剩余项保证均为用户|
|participation|用户组加入表|记录用户加入到用户组中的多对多关系|
|storage|存储介质表|记录用户（组）登录到系统的存储介质|
|storage_interface|存储介质接口表|分离storage的接口列以提升索引效率|
|storage_log|挂载日志表|分离storage的日志列以提升索引效率|
|filesystem|外围文件系统表|记录用户（组）挂载区域外的虚拟文件系统结构|
|mount|挂载点表|记录用户（组）在其外围文件系统中挂载介质的位置|
|symbolic_link|符号链接表|记录用户（组）在其外围文件系统中创建符号链接的位置|
|symbolic_link_target_path|符号链接路径表|分离symbolic_link表的path列以提升索引效率|
|recycle|回收站表|记录介质中被移入回收站的文件（夹）|
|recycle_path|回收站路径表|分离recycle表的path列以提升索引效率|
|sharing|链接分享表|记录用户（组）创建的分享链接|
|sharing_path|链接分享路径表|分离sharing表的path列以提升索引效率|
|invitation|邀请链接表|记录用户组加入邀请链接|
|admin|超级管理员表|记录超级管理员登录账户|

#### 2.1.3.3 表设计

- 用户（组）表（user）

|列名|类型|约束|解释|
|---|---|---|---|
|id|BIGINT|PK NOT NULL UNSIGNED AUTO_INCREMENT|主键|
|name|VARCHAR(64)|UNIQUE NOT NULL|用户名或用户组名|
|root| BIGINT | NOT NULL UNSIGNED | 根文件夹主键 |
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已被注销|

- 用户信息表（user_info）

|列名|类型|约束|解释|
|---|---|---|---|
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| user_id | BIGINT |UNIQUE NOT NULL UNSIGNED|用户主键|
|password|CHAR(64)|NOT NULL|密码（经SHA-256）|
|email|VARCHAR(64)| INDEX(16) |电子邮箱|
|phone_number|VARCHAR(16)| INDEX(8) |手机号码|
|allocated|INT|NOT NULL UNSIGNED|已被分配的存储空间（MB）|
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已被注销|

- 用户组加入表（participation）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| user_id | BIGINT | NOT NULL UNSIGNED UNIQUE(group_id, user_id) | 用户主键 |
|group_id| BIGINT | NOT NULL UNSIGNED UNIQUE(group_id, user_id) | 用户组主键 |
|donated| INT | NOT NULL UNSIGNED | 用户向用户组贡献的容量大小（MB） |
|privilege| INT | NOT NULL UNSIGNED | 权限 |
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已退出用户组|

- 存储介质表（storage）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| owner | BIGINT | NOT NULL UNSIGNED INDEX | 所有者主键 |
| name | VARCHAR(64)|  | 盘符 |
| occupation | INT | NOT NULL UNSIGNED | 介质占用空间（MB）（外部介质为0） |
| readonly | BOOL | NOT NULL | 是否以只读方式挂载 |
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已删除|

- 存储介质接口表（storage_interface）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED | 主键，与storage表一一对应 |
| interface | TEXT | NOT NULL | 介质访问接口（JSON格式） |

- 挂载日志表（storage_log）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED | 主键，与storage表一一对应 |
| update_time | TIMESTAMP | NOT NULL | 最近一次尝试挂载时间 |
| success | BOOL | NOT NULL | 最近一次尝试挂载是否成功 |
| log | TEXT | NOT NULL | 最近一次挂载日志 |

- 外围文件系统表（filesystem）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| name | VARCHAR(64)| NOT NULL | 文件夹名 |
|parent|BIGINT|UNSIGNED INDEX|父文件夹id|
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已删除|

- 挂载点表（mount）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| path | BIGINT | UNIQUE NOT NULL UNSIGNED | 挂载外围文件系统位置 |
| storage_id | BIGINT | NOT NULL UNSIGNED | 挂载介质主键 |
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已删除|

- 符号链接表（symbolic_link）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| parent | BIGINT | NOT NULL UNSIGNED INDEX | 符号链接父文件夹主键 |
| name | VARCHAR(64)| NOT NULL | 符号链接名 |
| target_user | BIGINT | NOT NULL UNSIGNED | 目标位置根目录的所有用户（组） |
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已删除|

- 符号链接路径表（symbolic_link_target_path）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED | 主键，与symbolic_link表一一对应 |
| path | TEXT | NOT NULL | 目标路径，以"/"分割文件夹名，开头结尾无"/" |

- 回收站表（recycle）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
|storage_id| BIGINT | NOT NULL UNSIGNED INDEX | 文件（夹）所在存储介质 |
|expire_time|TIMESTAMP|NOT NULL|执行彻底删除时间|
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已取消（完成删除或被用户恢复）|

- 回收站路径表（recycle_path）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED | 主键，与recycle表一一对应 |
| path | TEXT | NOT NULL | 目标路径，以"/"分割文件夹名，开头结尾无"/" |

- 链接分享表（sharing）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| owner | BIGINT | NOT NULL UNSIGNED INDEX | 所有者主键 |
| link | CHAR(26) | NOT NULL INDEX | 分享链接（只含ULID部分） |
| pwd | CHAR(4) | | 分享口令，为NULL则不存在 |
|expire_time|TIMESTAMP||共享结束时间（NULL表示不过期）|
|create_time|TIMESTAMP|NOT NULL|共享开始时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已取消（过期或用户手动取消）|

- 链接分享路径表（sharing_path）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED | 主键，与sharing表一一对应 |
| path | TEXT | NOT NULL | 目标路径，以"/"分割文件夹名，开头结尾无"/" |

- 邀请链接表（invitation）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| group_id | BIGINT | NOT NULL UNSIGNED INDEX | 用户组主键 |
| link | CHAR(26) | NOT NULL INDEX | 邀请链接（只含ULID部分） |
| pwd | CHAR(8) | | 邀请口令，为NULL则不存在 |
|expire_time|TIMESTAMP||邀请结束时间（NULL表示不过期）|
|create_time|TIMESTAMP|NOT NULL|邀请开始时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已取消（过期或用户手动取消）|

- 超级管理员表（admin）

| 列名 | 类型 | 约束 | 解释 |
| ---- | ---- | ---- | ---- |
| id   | BIGINT | PK NOT NULL UNSIGNED AUTO_INCREMENT | 主键 |
| account | VARCHAR(16) | UNIQUE NOT NULL | 管理员登录名 |
| password | CHAR(64) | NOT NULL | 密码（经SHA-256） |
|create_time|TIMESTAMP|NOT NULL|创建时间|
|update_time|TIMESTAMP|NOT NULL|上次更新时间|
|is_deleted|BOOL|NOT NULL|是否已被删除|

### 2.1.4 详细设计

#### 2.1.4.1 模块汇总

1. 用户管理模块：包含身份验证、用户信息变更、用户组信息变更等业务
    | 功能类名称                 | 功能简述                                                     |
    | -------------------------- | :----------------------------------------------------------- |
    | `AdminController.java`     | 管理员控制器层，实现管理登录，更改默认数据、修改用户权限等功能 |
    | `UserController.java`      | 用户控制器层，实现用户注册、登录、用户组CURD功能的鉴权操作   |
    | `UserService.java`         | 用户service层接口，负责业务逻辑处理，负责定义查找用户，获取用户/用户组实体类 |
    | `UserServiceImpl.java`     | 用户service层实现类，实现`UserService.java`方法              |
    | `UserMapper.java`          | 用户DAO层数据访问接口，负责定义数据库交互方法，如查询用户，添加用户/用户组，删除用户/用户组等功能 |
    | `User.java`                | 用户PO类，数据库user表的字段映射                             |
    | `UserDTO.java`             | 用户DTO类，与前端进行数据传输的对象，有`isGroup`             |
    | `UserInfoService.java`     | 用户信息service层接口，负责定义用户与用户组交互的信息变更    |
    | `UserInfoServiceImpl.java` | 用户信息service层接口实现类，实现``UserInfoService.java``方法 |
    | `UserInfoMapper.java`      | 用户信息DAO层数据访问接口，负责定义数据库交互方法，主要用来查找、获取用户信息 |
    | `UserInfo.java`            | 用户信息PO类，数据库user_info表字段映射                      |
    | `LoginDTO.java`            | 用户登陆DTO类，与前端进行数据传输对象                        |
    | `RegisterDTO.java`         | 用户注册DTO类，与前端进行数据传输对象                        |

2. 介质管理模块：包括介质添加、删除、上传、卸载等业务

   | 功能类名称                       | 功能简述                                                     |
   | -------------------------------- | ------------------------------------------------------------ |
   | `StorageController.java`         | 介质控制器层，负责调用Service层实现业务逻辑并进行鉴权，异常处理操作 |
   | `StorageService.java`            | 介质service层，负责业务逻辑处理，包括添加、删除介质，介质所有权转移，介质上传、下载等功能 |
   | `StorageServiceImpl.java`        | 介质service层实现类，实现`StorageService.java`方法           |
   | `StorageMapper.java`             | 介质DAO层数据访问接口，负责定义数据库交互方法，包括获取介质信息，插入、更新介质等方法 |
   | `MountMapper.java`               | 挂载点DAO层数据访问接口，负责定义数据库交互方法，包括介质挂载、卸载等方法 |
   | `Storage.java`                   | 介质PO类，数据库storage表字段映射                            |
   | `StorageConnectionLog.java`      | 介质挂载日志表，数据库storage_log表字段映射                  |
   | `StorageDTO.java`                | 介质DTO类，与前端进行数据交互的对象                          |
   | `StorageStatDTO.java`            | 介质状态信息DTO类，与前端进行数据交互的对象                  |
   | `StorageAccessor.java`           | 介质访问接口，记录介质添加，创建，移动等相关操作             |
   | `StorageAccessorRetryProxy.java` | 介质访问接口`StorageAccessor.java`实现类                     |

3. 文件管理模块

   | 功能类名称                  | 功能简述                                                     |
   | --------------------------- | ------------------------------------------------------------ |
   | `FileController.java`       | 文件管理控制器层，负责调用Service层实现业务逻辑并进行鉴权，异常处理操作 |
   | `FileService.java`          | 文件管理service层，负责业务逻辑处理，包括文件上传下载、文件/文件夹创建、移动、删除、重命名等等功能 |
   | `FileServiceImpl.java`      | 文件管理servce层实现类，实现`FileSerrvice.java`方法          |
   | `FilesystemMapper.java`     | 文件管理DAO层数据访问接口，负责定义数据库交互方法，包括查找子文件夹/文件，删除文件夹/文件等等数据交互 |
   | `SymbolicLinkMapper.java`   | 软链接DAO层数据访问接口。负责快捷方式链接其他文件夹的相关操作 |
   | `FileBO.java`               | 给定文件路径，返回对应文件夹、文件信息                       |
   | `SymbolicLink.java`         | 软链接类，指定链接目标等                                     |
   | `VirtualFolder.java`        | 虚拟文件夹类                                                 |
   | `FileStatDTO.java`          | 文件状态数据访问接口                                         |
   | `ListFilesResponseDTO.java` | 展示文件夹信息数据访问接口                                   |
   | `MkdirRequestDTO.java`      | 创建文件夹数据访问接口                                       |
   |                             |                                                              |
   |                             |                                                              |
   |                             |                                                              |

4. 工具类模块

   | 功能类名称               | 功能简述                           |
   | ------------------------ | ---------------------------------- |
   | `JwtUtil.java`           | JWT令牌工具类，实现token生成、解析 |
   | `PathUtil.java`          | 路径转换工具类                     |
   | `RegexUtil.java`         | 正则表达式匹配工具类               |
   | `Sha256Util.java`        | SHA256密码加密工具类               |
   | `VerifyingCodeUtil.java` | 验证码随机生成的工具类             |

#### 2.1.4.2 服务汇总

|No.|名称|业务范围|输入|输出|描述|
|--|:-:|---|--|--|:-:|
|1|邮箱验证码发送|身份验证|邮箱|成功与否|以电子邮件形式发送验证码以验证用户身份|
|2|邮箱验证码验证|身份验证|邮箱+验证码|正确与否|验证用户提供的电子邮件验证码是否正确|
|3|短信验证码发送|身份验证|手机号|成功语法|以短信形式发送验证码以验证用户身份|
|4|短信验证码验证|身份验证|手机号+验证码|正确与否|验证用户提供的短信验证码是否正确|
|5|用户登录|身份验证|(用户名\|手机号\|邮箱)+密码|token令牌|验证用户身份并返回临时登录令牌|
|6|用户注册|用户信息变更|用户名+(手机号\|邮箱)+密码|用户DO|创建新用户|
|7|创建用户组|用户信息变更|用户DO+用户组名|用户组DO|创建新用户组|
|8|加入用户组|用户信息变更|用户DO+用户组DO|成功与否|用户加入新的用户组|
|9|退出用户组|用户信息变更|用户DO+用户组DO|成功与否|用户退出用户组|
|10|删除用户组|用户组信息变更|用户DO+用户组DO|成功与否|创建者删除用户组|
|11|创建文件夹|文件管理|路径path + root文件夹所有者+virtual|成功与否|用户创建文件夹|
|12|展示文件夹目录| 文件管理       |路径path + root文件夹所有者|成功与否|列出目前工作目录所含的文件及子目录|
|13|删除文件/文件夹|文件管理|FileBO 文件|成功与否|删除给定路径下的文件/文件夹|
|14|介质挂载|介质管理|FileBO file + 介质主键|成功与否|将指定介质挂载到对应文件夹上|
|15|介质卸载|介质管理|FileBO file|成功与否|将指定介质卸载|
|||||||
|||||||

#### 2.1.4.3 API汇总

| 序号 | 请求方式 |            API 地址            |             描述             |
| ---- | :------: | :----------------------------: | :--------------------------: |
| 1    |   POST   |     /api/user/email-verify     |        发送邮箱验证码        |
| 2    |   POST   |       /api/user/register       |           注册用户           |
| 3    |   POST   |        /api/user/login         |           用户登录           |
| 4    |   POST   |    /api/user/participation     |          创建用户组          |
| 5    |   POST   | /api/user/join-group/{groupId} |          加入用户组          |
| 6    |  DELETE  | /api/user/exit-group/{groupId} |          退出用户组          |
| 7    |  DELETE  |    /api/user/participation     |          删除用户组          |
| 8    |   POST   |      /api/storage/upload       |           添加介质           |
| 9    |   GET    |          /api/file/ls          | 展示给定文件路径下的文件信息 |
| 10   |   POST   |        /api/file/mkdir         |          创建文件夹          |
| 11   |   GET    |       /api/user/userinfo       |         获取用户信息         |
| 12   |  DELETE  |           /api/file            |  删除当前路径的文件/文件夹   |
| 13   |   POST   |       /api/storage/mount       |           介质挂载           |
| 14   |   POST   |      /api/storage/unmount      |           介质卸载           |
| 15   |   PUT    |     /api/storage/transfer      |           转移介质           |
| 16   |  DELETE  |          /api/storage          |           删除介质           |
| 17   |   GET    |        /api/storage/ls         |    获取指定用户拥有的介质    |
|      |          |                                |                              |
|      |          |                                |                              |
|      |          |                                |                              |
|      |          |                                |                              |
|      |          |                                |                              |
|      |          |                                |                              |
|      |          |                                |                              |


#### 2.1.4.4 API设计

1. POST /api/user/email-verify

   - 请求头参数

      | 序号   | 参数名 | 类型 | 参数示例值 |
      | :----: | :----: | :--: | :--: |
      |1| Content-Type | string | application/json |

   - 请求体参数（JSON格式）

      | 序号   | 参数名 | 类型 | 样例值 | 描述 |
      | :----: | :----: | :--: | :----: | :--: |
      |1| account | string | 123@abc.com | 邮箱 |

   - 返回值

     1. 成功（200）

        ```json
        {
            "code": 200,
            "message": "操作成功",
            "data": null 
        }
        ```

     2. 邮箱格式错误（404）

        ```json
        {
            "code": 404,
            "message": "邮箱格式错误",
            "data": null 
        }
        ```

     3. 该邮箱已注册，请更改邮箱（401）

        ```json
        {
            "code": 401,
            "message": "该邮箱已注册，请更改邮箱",
            "data": null 
        }
        ```

2. POST /api/user/register
   - 请求头参数

       | 序号   | 参数名 | 类型 | 参数示例值 |
       | :----: | :----: | :--: | :--: |
       |1| `Content-Type` | `String` | `application/json` |
   - 请求体参数（JSON格式）

       | 序号   | 参数名 | 类型 | 样例值 | 描述 |
       | :----: | :----: | :--: | :----: | :--: |
       |1| `username` | `String` | `zhangsan` | 用户名 |
     |2| `password` | `String` | `123456` | 密码 |
     |3| `verifyNumber` | `String` | `123@qq.com|18531511215` | 邮箱或电话号码 |
     |4| `verification` | `String` | `654121` | 验证码 |

   - 返回值

     1. 成功（200）

        ```json
        {
            "code": 200,
            "message": "操作成功",
            "data": {
                "id": 1,
                "name": "zhangsan",
                "root": 2
            } 
        }
        ```
     
     2. 验证码错误（401）
     
        ```json
        {
            "code": 401,
            "message": "验证码错误",
            "data": null 
        }
        ```
     
     3. 该用户名已存在（409）
     
        ```json
        {
            "code": 409,
            "message": "该用户名已存在",
            "data": null 
        }
        ```

3. POST /api/user/login

    - 请求头参数

         | 序号   | 参数名 | 类型 | 参数示例值 |
         | :----: | :----: | :--: | :--: |
         |1| `Content-Type` | `String` | `application/json` |
         
    - 请求体参数

         | 序号 |     参数名      |   类型   |           样例值            |          描述          |
         | :--: | :-------------: | :------: | :-------------------------: | :--------------------: |
         |  1   | `userLoginName` | `String` | `wang|12@a.com|12345678945` | 用户名或电话号码或邮箱 |
         |  2   |   `password`    | `String` |          `123456`           |          密码          |

    - 返回值

         1. 成功（200）

            ```json
            {
                "code": 200,
                "message": "操作成功",
                "data": null 
            }
            ```

         2. 用户名或密码错误（401）

            ```json
            {
                "code": 401,
                "message": "用户名或密码错误",
                "data": null 
            }
            ```
         

4. POST /api/user/participation

   - 请求头参数

     | 序号   | 参数名 | 类型 | 参数示例值 |
     | :----: | :----: | :--: | :--: |
     |1| `Content-Type` | `String` | `application/json` |
     |2| ` Authorization` | `String` | token值 |
     
   - 请求体参数

     | 序号 | 参数名  |   类型   | 样例值 |
     | :--: | :-----: | :------: | :----: |
     |  1   | `group` | `String` | `1zu`  |

   - 返回值

     1. 成功（200）
     	 ```json
     	{
     	    "code": 200,
     	    "message": "操作成功",
     	    "data": {
     	        "id": 4,
     	        "name": "1zu",
     	        "root": 3
     	    } 
     	}
     	```
     
     2. 登陆过期，添加失败（401）
     	```json
        {
        "code": 401,
        "message": "登陆过期，添加失败",
        "data": null
        }
        ```

5. POST /api/user/join-group

    - 请求头参数

      | 序号 |      参数名      |   类型   |     参数示例值     |
      | :--: | :--------------: | :------: | :----------------: |
      |  1   |  `Content-Type`  | `String` | `application/json` |
      |  2   | ` Authorization` | `String` |      token值       |

    - 路径参数

      | 序号 |  参数名   |     类型     | 样例值 |
      | :--: | :-------: | :----------: | :----: |
      |  1   | `groupId` | `BigInteger` |   6    |

    - 返回值

         1. 成功（200）

            ```json
            {
                "code": 200,
                "message": "操作成功",
                "data": null
            }
            ```

         2. 添加失败 请先输入要添加的小组信息（401）

            ```json
            {
                "code": 401,
                "message": "添加失败 请先输入要添加的小组信息",
                "data": null
            }
            ```

         3. 添加失败 输入的小组不存在（404）

            ```json
            {
                "code": 404,
                "message": "添加失败 您输入的小组不存在",
                "data": null
            }		
            ```

6. DELETE /api/user/exit-group/{groupId}

   - 请求头参数

     | 序号 | 参数名           | 类型     | 参数示例值         |
     | ---- | ---------------- | -------- | ------------------ |
     | 1    | `Content-Type`   | `String` | `application/json` |
     | 2    | ` Authorization` | `String` | token值            |

   - 路径参数

     | 序号 | 参数名    | 类型         | 样例值 |
     | ---- | --------- | ------------ | ------ |
     | 1    | `groupId` | `BigInteger` | 6      |

   - 返回值

     1. 成功（200）

        ```json
        {
            "code": 200,
            "message": "操作成功",
            "data": null
        }
        ```

     2. 退出失败 请先输入要退出的小组信息（401）

        ```json
        {
            "code": 401,
            "message": "退出失败 请先输入要退出的小组信息",
            "data": null
        }
        ```

     3. 退出失败 您已不在该小组（404）

        ```json
        {
            "code": 404,
            "message": "退出失败 您已不在该小组",
            "data": null
        }
        ```

7. DELETE /api/user/participation

   - 请求头参数

     | 序号 |      参数名      |   类型   |     参数示例值     |
     | :--: | :--------------: | :------: | :----------------: |
     |  1   |  `Content-Type`  | `String` | `application/json` |
     |  2   | ` Authorization` | `String` |      token值       |

   - 路径参数

     | 序号 | 参数名    | 类型         | 样例值 |
     | ---- | --------- | ------------ | ------ |
     | 1    | `groupId` | `BigInteger` | 6      |

   - 返回值

        1. 成功（200）

           ```json
           {
               "code": 200,
               "message": "操作成功",
               "data": null
           }
           ```

     2. 删除失败

8. POST /api/storage/upload

   - 请求头参数

     | 序号 | 参数名           | 类型     | 参数示例值         |
     | ---- | ---------------- | -------- | ------------------ |
     | 1    | `Content-Type`   | `String` | `application/json` |
     | 2    | ` Authorization` | `String` | token值            |

   - 请求体参数

     | 序号 | 参数名             | 类型         | 样例值                           |
     | ---- | ------------------ | ------------ | -------------------------------- |
     | 1    | `owner`            | `BigInteger` | `介质所有者                      |
     | 2    | `name`             | `String`     | 盘符名                           |
     | 3    | `occupation`       | `Integer`    | 介质占用空间（MB）(外部介质为0） |
     | 4    | `readonly`         | `Boolean`    | 是否以只读方式挂载               |
     | 5    | `storageInterface` | `String`     | 介质访问接口（路径）             |

   - 返回值

        1. 成功（200）

           ```json
           {
               "code": 200,
               "message": "操作成功",
               "data": null
           }
           ```

9. GET /api/file/ls

   - 请求头参数

     | 序号 | 参数名           | 类型     | 参数示例值         |
     | ---- | ---------------- | -------- | ------------------ |
     | 1    | `Content-Type`   | `String` | `application/json` |
     | 2    | ` Authorization` | `String` | token值            |

   - 查询参数

     | 序号 | 参数名 | 类型         | 样例值                      |
     | ---- | ------ | ------------ | --------------------------- |
     | 1    | `path` | `String`     | 请求路径：`aaa/bbb/ccc.txt` |
     | 2    | `root` | `BigInteger` | 文件夹所有者Id：`1`         |

   - 返回值

     1. 成功（200）

        ```json
        {
            "code": 200,
            "message": "操作成功",
            "data": {
                "dir": {
                    "name": "ShareCloud-backend",
                    "type": "dir",
                    "lastModified": "2024-08-31T14:13:25.441+00:00",
                    "mount": {
                        "id": 1,
                        "owner": {
                            "id": 1,
                            "name": "tangxin",
                            "isGroup": false
                        },
                        "name": "testStorage",
                        "readonly": false,
                        "type": "LocalStorage",
                        "connectionInfo": "/data/localStorage"
                    }
             	},
                "children": [
                    {
                        "name": "README.md",
                        "type": "file",
                        "size": 22,
                        "lastModified": "2024-08-31T14:13:25.437+00:00"
                    },
                     {
                        "name": "Dockerfile",
                        "type": "file",
                        "size": 272,
                        "lastModified": "2024-08-31T14:13:25.437+00:00"
                    }
                ]    
            }
        }
        ```

10. POST /api/file/mkdir

    - 请求头参数

      | 序号 | 参数名           | 类型     | 参数示例值         |
      | ---- | ---------------- | -------- | ------------------ |
      | 1    | `Content-Type`   | `String` | `application/json` |
      | 2    | ` Authorization` | `String` | token值            |

    - 请求体参数

      | 序号 | 参数名    | 类型         | 样例值                  |
      | ---- | --------- | ------------ | ----------------------- |
      | 1    | `path`    | `String`     | 请求路径：`aaa/bbb`     |
      | 2    | `root`    | `BigInteger` | 文件夹所有者Id：`1`     |
      | 3    | `virtual` | `Boolean`    | 是否为虚文件夹：`false` |

    - 返回值

      1. 成功（200）

         ```json
         {
             "code": 200,
             "message": "操作成功",
             "data": null
         }
         ```

11. GET /api/user/userinfo

     - 请求头参数

       | 序号 | 参数名          | 类型     | 参数示例值         |
       | ---- | --------------- | -------- | ------------------ |
       | 1    | `Content-Type`  | `String` | `application/json` |
       | 2    | `Authorization` | `String` | token值            |

     - 返回值

       1. 成功（200）

          ```json
          {
              "code": 200,
              "message": "操作成功",
              "data": {
                  "name": "ta",
                  "email": "1@abc.com",
                  "allocated": 1024
              }
          }
          ```

12. DELETE /api/file

     - 请求头参数

       | 序号 |     参数名      |   类型   |     参数示例值     |
       | :--: | :-------------: | :------: | :----------------: |
       |  1   | `Content-Type`  | `String` | `application/json` |
       |  2   | `Authorization` | `String` |      token值       |

     - 请求体参数

       | 序号 | 参数名 | 类型         | 示例值        |
       | ---- | ------ | ------------ | ------------- |
       | 1    | `path` | `String`     | a/b/c/abc.txt |
       | 2    | `root` | `BigInteger` | 1             |

     - 返回值

       1. 成功（200）

          ```json
          {
              "code": 200,
              "message": "操作成功",
              "data": null
          }
          ```

       2. 文件不存在（404）

          ```json
          {
              "code": 404,
              "message": "文件不存在",
              "data": null
          }
          ```

13. POST /api/storage/mount

     - 请求头参数

       | 序号 | 参数名          | 类型     | 参数示例值         |
       | ---- | --------------- | -------- | ------------------ |
       | 1    | `Content-Type`  | `String` | `application/json` |
       | 2    | `Authorization` | `String` | token值            |

     - 请求体参数

       | 序号 | 参数名    | 类型         | 示例值        |
       | ---- | --------- | ------------ | ------------- |
       | 1    | `path`    | `String`     | a/b/c/abc.txt |
       | 2    | `root`    | `BigInteger` | 1（可以不传） |
       | 3    | `storage` | `BigInteger` | 存储介质ID    |

     - 返回值

       1. 成功（200）

          ```json
          {
              "code": 200,
              "message": "操作成功",
              "data": null
          }
          ```

14. POST /api/storage/unmount

     - 请求头参数

       | 序号 | 参数名          | 类型     | 参数示例值         |
       | ---- | --------------- | -------- | ------------------ |
       | 1    | `Content-Type`  | `String` | `application/json` |
       | 2    | `Authorization` | `String` | token值            |

     - 请求体参数

       | 序号 | 参数名 | 类型         | 示例值        |
       | ---- | ------ | ------------ | ------------- |
       | 1    | `path` | `String`     | a/b/c/abc.txt |
       | 2    | `root` | `BigInteger` | 1（可以不传） |

     - 返回值

       1. 成功（200）

          ```json
          {
              "code": 200,
              "message": "操作成功",
              "data": null
          }
          ```

       2. 该文件没有挂载介质（404）

          ```json
          {
              "code": 404,
              "message": "该文件没有挂载介质",
              "data": null
          }
          ```

15. PUT /api/storage/transfer

    - 请求头参数

      | 序号 | 参数名          | 类型     | 参数示例值         |
      | ---- | --------------- | -------- | ------------------ |
      | 1    | `Content-Type`  | `String` | `application/json` |
      | 2    | `Authorization` | `String` | token值            |

    - 请求体参数

      | 序号 | 参数名      | 类型         | 示例值   |
      | ---- | ----------- | ------------ | -------- |
      | 1    | `srcId`     | `BigInteger` | 可以不传 |
      | 2    | `dstId`     | `BigInteger` | 24       |
      | 3    | `storageId` | `BigIntger`  | 3        |

    - 返回值

      1. 成功（200）

         ```json
         {
             "code": 200,
             "message": "操作成功",
             "data": null
         }
         ```

      2. 您无权限转移他人介质|您无权限转移介质到目标用户组（403）

      3. 您未加入介质的所在用户组，无法转移介质（404）

16.  DELETE /api/storage

    - 请求头参数

      | 序号 | 参数名          | 类型     | 参数示例值         |
      | ---- | --------------- | -------- | ------------------ |
      | 1    | `Content-Type`  | `String` | `application/json` |
      | 2    | `Authorization` | `String` | token值            |

    - 请求体参数

      | 序号 | 参数名      | 类型         | 示例值 |
      | ---- | ----------- | ------------ | ------ |
      | 1    | `owner`     | `BigInteger` | 1      |
      | 2    | `storageId` | `BigInteger` | 24     |

    - 返回值

      1. 成功（200）

         ```json
         {
             "code": 200,
             "message": "操作成功",
             "data": null
         }
         ```

      2. 您未加入该用户组，无法删除该介质（404）

      3. 您无权限删除该介质（403）

17.  GET /api/storage/ls

    - 请求头参数

      | 序号 | 参数名          | 类型     | 参数示例值         |
      | ---- | --------------- | -------- | ------------------ |
      | 1    | `Content-Type`  | `String` | `application/json` |
      | 2    | `Authorization` | `String` | token值            |
    
    - 查询参数
    
      | 序号 | 参数名   | 类型         | 示例值 |
      | ---- | -------- | ------------ | ------ |
      | 1    | `userId` | `BigInteger` | 1      |
    
    - 返回值
    
      1. 成功（200）
    
         ```json
         {
             "code": 200,
             "message": "操作成功",
             "data": {
                 "resp":[
                     {
                         "user": {
                             "id": 1,
                             "name": "sss",
                             "isGroup": false
                         },
                         "storages":[
                             {
                                 "id": 2,
                                 "name": "ssa",
                                 "readonly": true,
                                 "type": "minio",
                                 "connectionInfo": ""
                             }
                         ]
                     }
                 ]
             }
         }
         ```
    

## 2.2 系统主要模块实现

1. 邮箱验证码发送

   ```mermaid
   graph TD
       S("开始")-->A["生成随机6位验证码"]-->B["存(邮箱: 验证码)进Redis"]-->C["调用邮件发送API发送验证邮件"]-->E("结束")
   ```

2. 邮箱验证码验证

   ```mermaid
   graph TD
       S("开始")-->A(["检查Redis中是否存在(邮箱: 验证码)"])
       A--是-->E1("返回true")
       A--否-->E2("返回false")
   ```

3. 短信验证码发送（同1，略）

4. 短信验证码验证（同2，略）

5. 用户登录

   ```mermaid
   graph TD
       S("开始")-->A(["检查user中是否存在指定用户且密码正确"])
       A--是-->B["生成Session"]-->C["将Session存入Redis"]-->E1("返回Session")
       A--否-->E2("返回登录失败")
   ```

6. 用户注册

   ```mermaid
   graph TD
       S("开始")-->A["在filesystem中插入新项"]-->B["在user中插入新项，设root=filesystem新项主键"]-->C["在user_info中插入新项，设user_id=user新项主键"]-->E("结束")
   ```

7. 创建用户组

   ```mermaid
   graph TD
       S("开始")-->A["在filesystem中插入新项"]-->B["在user中插入新项，设root=filesystem新项主键"]-->C["在participation中插入新项，设user_id=用户ID，group_id=user新项主键"]-->E("结束")
   ```

8. 加入用户组

   ```mermaid
    graph TD
    S("开始")-->B["在user中根据group_id查找用户组是否存在"]
    B--否-->返回加入失败
    B--是-->A["在participation中插入新项，设user_id=用户ID，group_id=用户组ID"]-->E("结束")
   ```

9. 退出用户组

    ```mermaid
    graph TD
    S("开始")-->A["查找participation中指定user_id是否group_id对应"]
    A--是-->B["将participation中user_id与group_id对应的is_deleted = 1"]
    A--否-->C["返回退出失败"]
    ```

10. 删除用户组

    ```mermaid
    graph TD
    S("开始")-->A["根据userId获取privilege"]-->D["privilege & 1 == 1 ?"]
    D--是-->B["在participation中更新所有指定groupId的is_deleted = 1"]-->E["在user表中删除id == groupId的项"]-->F["删除文件操作：user_info、participation、filesystem、mount"]
    D--否-->C["无权限删除"]
    ```

11. 创建文件夹

12. 展示文件/文件夹子目录

13. 删除文件/文件夹

14. 介质挂载

     ```mermaid
     graph TD
     S("开始")-->A["检查给定文件路径结点parent是否是虚拟文件夹"]
     B["mkdir"]
     A--是-->B--循环-->A
     A--否-->C
     C["在mount中插入新项，设path=parent，storage_id = storgaeId"]
     ```

     

15. 介质卸载

     ```mermaid
     graph TD
     S("开始")-->A["检查当前路径下文件夹是否有虚拟文件夹"]
     A--是-->B["当前路径没有挂载介质"]
     A--否-->C["在mount中将path = getVirtualFolder.id的介质is_delted = 1"]
     ```

     



## 2.3 遇到的问题及解决

# 3 总结

## 3.1 体会和收获

## 3.2 意见和建议
